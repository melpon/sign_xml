defmodule SignXML.Verifier do
    ...
    Libxml.XPath.safe_new_context(node.doc, fn ctx ->
      ...
      Libxml.XPath.safe_eval(ctx, xpath, fn obj ->
        ...
        case obj.type do
          ...
          :string ->
            {:string, Libxml.Char.extract(obj.content).content}
        end
      end)
    end)
  end
  ...
  defp get_attribute_value(%Libxml.Node{} = node) do
    ...
    %Libxml.Node{type: :text_node, more: %Libxml.Node.Default{content: content}} =
      Libxml.Node.extract(children)
      ...
      String.starts_with?(uri_value, "#") ->
        ...
        try do
          for id_attribute <- id_attributes do
            xpath_query =
              "//*[@*[local-name()='#{id_attribute}']='#{String.trim_leading(uri_value, "#")}']"
            ...
            case length(results) do
              ...
              1 ->
                throw(Enum.fetch!(results, 0))

              _ ->
                raise "Ambiguous reference URI #{uri_value} resolved to #{length(results)} nodes"
            end
          end
    ...
    result =
      case algorithm do
        "http://www.w3.org/TR/2001/REC-xml-c14n-20010315" ->
          SignXML.C14N.c14n(payload, :c14n_1_0, inclusive_ns_prefixes, false)

        "http://www.w3.org/TR/2001/REC-xml-c14n-20010315#WithComments" ->
          SignXML.C14N.c14n(payload, :c14n_1_0, inclusive_ns_prefixes, true)

        "http://www.w3.org/2001/10/xml-exc-c14n#" ->
          SignXML.C14N.c14n(payload, :c14n_exclusive_1_0, inclusive_ns_prefixes, false)

        "http://www.w3.org/2001/10/xml-exc-c14n#WithComments" ->
          SignXML.C14N.c14n(payload, :c14n_exclusive_1_0, inclusive_ns_prefixes, true)

        "http://www.w3.org/2006/12/xml-c14n11" ->
          SignXML.C14N.c14n(payload, :c14n_1_1, inclusive_ns_prefixes, false)

        "http://www.w3.org/2006/12/xml-c14n11#WithComments" ->
          SignXML.C14N.c14n(payload, :c14n_1_1, inclusive_ns_prefixes, true)
        ...
        payload =
          Enum.reduce(transforms, payload, fn transform, payload ->
            inclusive_namespaces_prefix_list =
              find_single_or_zero_node(
                transform,
                "./*[local-name()='InclusiveNamespaces']/@PrefixList"
              )

            inclusive_ns_prefixes =
              if inclusive_namespaces_prefix_list == nil do
                []
              else
                inclusive_namespaces_prefix_list
                |> get_attribute_value()
  ...
  defp get_signature_digest_method("http://www.w3.org/2001/04/xmldsig-more#rsa-sha256"),
    do: {:rsa, :sha256}

  defp get_signature_digest_method("http://www.w3.org/2001/04/xmldsig-more#ecdsa-sha256"),
    do: {:ecdsa, :sha256}
  ...
  defp get_signature_digest_method("http://www.w3.org/2001/04/xmldsig-more#rsa-sha224"),
    do: {:rsa, :sha224}

  defp get_signature_digest_method("http://www.w3.org/2001/04/xmldsig-more#rsa-sha384"),
    do: {:rsa, :sha384}

  defp get_signature_digest_method("http://www.w3.org/2001/04/xmldsig-more#rsa-sha512"),
    do: {:rsa, :sha512}

  defp get_signature_digest_method("http://www.w3.org/2001/04/xmldsig-more#ecdsa-sha1"),
    do: {:ecdsa, :sha}

  defp get_signature_digest_method("http://www.w3.org/2001/04/xmldsig-more#ecdsa-sha224"),
    do: {:ecdsa, :sha224}

  defp get_signature_digest_method("http://www.w3.org/2001/04/xmldsig-more#ecdsa-sha384"),
    do: {:ecdsa, :sha384}

  defp get_signature_digest_method("http://www.w3.org/2001/04/xmldsig-more#ecdsa-sha512"),
    do: {:ecdsa, :sha512}

  defp get_signature_digest_method("http://www.w3.org/2009/xmldsig11#dsa-sha256"),
    do: {:dss, :sha256}
  ...
  defp check_digest(root, opts) do
    ...
    for reference <- find_all_node(signed_info, "*[local-name()='Reference']") do
      ...
      {:string, digest_value} =
        find_all(reference, "string(*[local-name()='DigestValue'][text()])")
  ...
  @record_opts [from_lib: "public_key/include/public_key.hrl"]
  Record.defrecordp(:otpCertificate, Record.extract(:OTPCertificate, @record_opts))
  Record.defrecordp(:otpTBSCertificate, Record.extract(:OTPTBSCertificate, @record_opts))
  Record.defrecordp(:rsaPublicKey, Record.extract(:RSAPublicKey, @record_opts))
  Record.defrecordp(:publicKeyAlgorithm, Record.extract(:PublicKeyAlgorithm, @record_opts))
  Record.defrecordp(:dssSigValue, Record.extract(:"Dss-Sig-Value", @record_opts))

  Record.defrecordp(
    :otpSubjectPublicKeyInfo,
    Record.extract(:OTPSubjectPublicKeyInfo, @record_opts)
  )
  ...
  defp get_public_key(der_certs, opts) do
    ...
    {:ok, {_public_key_info, _policy_tree}} =
      :public_key.pkix_path_validation(trusted_cert, der_certs, [])
  ...
  defp verify_signature(root, opts) do
    ...
    x509_data =
      find_single_or_zero_node(signature, "*[local-name()='KeyInfo']/*[local-name()='X509Data']")
    ...
    cond do
          ...
          case Keyword.fetch(opts, :x509_cert) do
            :error ->
              if x509_data == nil do
                ...
                der_certs =
                  Enum.map(x509certs, fn x509cert ->
      ...
      true ->
        key_value =
          find_single_node(signature, "*[local-name()='KeyInfo']/*[local-name()='KeyValue']")
        ...
        cond do
          String.contains?(signature_alg, "dsa-") ->
            ...
            unless :crypto.verify(:dss, digest_method, signed_info_c14n, der_signature, pqgy) do
          ...
          String.contains?(signature_alg, "rsa-") ->
            ...
            key = [exponent, modules]

            unless :crypto.verify(:rsa, digest_method, signed_info_c14n, raw_signature, key) do
